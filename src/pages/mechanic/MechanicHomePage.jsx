      ...
      {loading ? (
        ...
        <div className="space-y-6 pt-6 border-t border-slate-200 dark:border-slate-800">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-2xl bg-orange-100 dark:bg-orange-950/80 text-orange-600 dark:text-orange-400 flex items-center justify-center font-bold shadow-sm">
                <SirenLight size="xs" variant="ambulance" animated={true} />
              </div>
              <h2 className="text-xl sm:text-3xl font-black text-slate-900 dark:text-white font-heading tracking-tight">
                Incoming Driver SOS Emergency Requests
              </h2>
            </div>
            <span className="text-xs font-black font-mono px-4 py-2 bg-orange-50 dark:bg-orange-950/60 text-orange-700 dark:text-orange-400 rounded-full border border-orange-200 dark:border-orange-800">
          ...
          {requests.length === 0 ? (
            <div className="clean-card dark:bg-slate-900 dark:border-slate-800 p-14 text-center text-slate-500 dark:text-slate-400 space-y-3 border-2 border-slate-200 shadow-sm rounded-3xl">
              <Wrench className="w-14 h-14 text-slate-300 dark:text-slate-600 mx-auto" />
              <p className="text-lg font-black text-slate-800 dark:text-slate-200 font-heading">No active requests nearby right now.</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8">
              {requests.map((req) => (
                <div 
                  key={req.id} 
                  className="clean-card dark:bg-slate-900 dark:border-slate-800 p-7 space-y-5 flex flex-col justify-between border-2 border-slate-200 dark:border-slate-800 hover:border-orange-400 dark:hover:border-orange-500 hover:shadow-xl transition-all rounded-3xl relative overflow-hidden"
                >
                  {/* Top Accent Strip */}
                  ...
                  <div className="space-y-4 pt-1">
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="text-base sm:text-lg font-black text-slate-900 dark:text-white flex items-center gap-2 font-heading">
                      ...
                      <span className="text-xs font-black font-mono text-orange-700 dark:text-orange-300 bg-orange-50 dark:bg-orange-950/60 px-3.5 py-1.5 rounded-xl border border-orange-200 dark:border-orange-800 whitespace-nowrap shadow-xs">
                        {req.distance} away
                      </span>
                    </div>

                    {/* Rich Vehicle Identification Card (Input by Driver) */}
                    <div className="p-4 bg-gradient-to-r from-slate-50 to-slate-100/60 dark:from-slate-800/80 dark:to-slate-800/40 rounded-2xl border border-slate-200 dark:border-slate-700 space-y-2">
                      <div className="flex items-center justify-between">
                        <span className="text-[11px] font-bold text-slate-500 dark:text-slate-400 font-mono uppercase flex items-center gap-1.5">
                          <Car className="w-4 h-4 text-orange-600 dark:text-orange-400" />
                          Vehicle Category
                        </span>
                        <span className="font-mono text-xs font-black uppercase px-2.5 py-1 rounded-md bg-slate-900 text-amber-400 dark:bg-black border border-slate-700 tracking-wider">
                    ...
                    <div className="p-4 sm:p-5 bg-orange-50/50 dark:bg-slate-800/60 rounded-2xl space-y-2.5 text-xs border border-orange-100 dark:border-slate-700">
                      <div>
                        <span className="text-orange-800 dark:text-orange-300 font-mono text-[11px] uppercase block font-black">
                          Reported Breakdown:
                        </span>
                        <p className="font-black text-slate-900 dark:text-white text-sm sm:text-base font-heading mt-1">
                          {req.problemType || req.problem}
                        </p>
                      </div>

                      {req.notes && (
                        <div className="p-3 bg-white dark:bg-slate-900/80 rounded-xl border border-orange-200/60 dark:border-slate-700 text-slate-700 dark:text-slate-300 text-xs font-medium italic leading-relaxed">
                          "{req.notes}"
                        </div>
                      )}

                      <div className="flex items-center justify-between pt-1.5 text-slate-500 dark:text-slate-400">
                  ...
                  <div className="grid grid-cols-2 gap-3.5 pt-4 border-t border-slate-100 dark:border-slate-800">
                    <button
                      ...
                      className="btn-secondary py-3.5 text-xs sm:text-sm font-bold flex items-center justify-center gap-2 rounded-2xl"
                    >
                    ...
                    <button
                      ...
                      className="btn-primary py-3.5 text-xs sm:text-sm font-black flex items-center justify-center gap-2 shadow-md rounded-2xl"
                    >
